// server/tasks/cron/bookings-report.ts
import ExcelJS from 'exceljs'
import path from 'node:path'
import fs from 'node:fs'
import { logError } from '../../utils/logger'
import { useDb } from '../../utils/db'
import { sendBookingsReportEmail } from '../../utils/email'
...
export default defineTask({
  meta: {
    ...
    description: 'Genera reporte Excel de reservas de la semana y lo envía por email'
  },
  async run() {
    let client
    try {
      ...
      if (bookings.length > 0) {
        ...
        filePath = path.join(dir, `reservas-${dateStr}.xlsx`)
      ...
      console.log(`[CRON:bookings-report] Reservas: ${bookings.length}, archivo: ${filePath || 'no generado (sin reservas)'}, email enviado: ${reportEmail.success}`)
      ...
      logError({
        endpoint: 'task:cron:bookings-report',
        method: 'TASK',
        error: String(error),
